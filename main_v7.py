...
def log_signal_rejection(engine, asset, direction, win_prob, threshold, regime, reason, bot="sniper"):
    """
...
# Used to compute np.percentile-based gate that scales with the live model distribution
asset_score_buffer: dict = {asset: [] for asset in config.TARGET_ASSETS}
SCORE_BUFFER_SIZE = 50       # Keep last N predictions per asset
DYNAMIC_PERCENTILE_GATE = 85 # Top 15th percentile passes (top 15% of recent scores)
...
def track_rejected_signal(asset, direction, win_prob, threshold, entry_price, atr_val, reason, regime="NORMAL"):
    ...
    log_signal_rejection(global_db_engine, asset, direction, win_prob, threshold, regime, reason, bot="sniper")
        ...
        async with self.state_lock:
            ...
            for trade in [t for t in active_trades if t['asset'] == symbol]:
                ...
                if is_closed:
                    ...
                    print(f"  [{status_icon.replace('🟢','PROFIT').replace('🔴','LOSS')} TRADE CLOSED] {symbol} {trade['direction']} | PnL: ${pnl:+.2f} | Exit: {exit_reason} | Held: {hold_str} | AI: {ai_prob:.1f}%")

                    msg = (
                        f"{status_icon} 🔔 *[V8.2] TRADE CLOSED — {result}*\n"
